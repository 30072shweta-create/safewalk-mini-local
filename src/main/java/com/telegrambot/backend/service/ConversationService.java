 ...
 * This handles registration, trip setup, contact management, and other non-single-command flows.
 */
@Service
public class ConversationService {
     ...
     * Determines if the update is a command or a continuation of an existing conversation.
     *
    ...
    private void handleConversationInput(long chatId, String telegramId, String input) {
        ...
        switch (currentState) {
            ...
            case IDLE:
            default:
                telegramBotService.sendMessage(chatId, "I'm not sure what to do with that. Please use a command like /new_trip or /help.");
                break;
        }
    }
...
userRepository.save(user);
        ...
        telegramBotService.sendMessage(chatId, "Thank you, " + name + ". What is your phone number (e.g., +1234567890)?");
    }
    ...
    private void handleRegisterPhone(long chatId, String telegramId, String phone) {
        ...
        if (userOpt.isPresent()) {
            ...
            if (phone.matches("^\\+?[0-9]{7,15}$")) {
                ...
                telegramBotService.sendMessage(chatId, "Got the phone number. Finally, what is your gender (MALE/FEMALE/OTHER)?");
            } else {
                telegramBotService.sendMessage(chatId, "That doesn't look like a valid phone number. Please provide a number starting with the country code (e.g., +1234567890).");
            }
        } else {
    ...
    private void handleRegisterGender(long chatId, String telegramId, String gender) {
        ...
        if (normalizedGender.equals("MALE") || normalizedGender.equals("FEMALE") || normalizedGender.equals("OTHER")) {
            ...
            if (userOpt.isPresent()) {
                ...
                conversationStates.put(chatId, ConversationState.IDLE);
                telegramBotService.sendMessage(chatId, "Registration complete! You are all set up. You can now start a trip with /new_trip or manage contacts with /contacts.");
            } else {
                telegramBotService.sendMessage(chatId, "Error: User data missing. Please start again with /start.");
            }
        } else {
    ...
    private void handleTripSource(long chatId, String telegramId, String sourceAddress) {
        // NOTE: This logic REQUIRES TripRepository and the ability to call the core SafeWalk backend
        ...
        // 2. Call SafeWalk backend to get route options for source/destination (once destination is known).
        
        // For now, we simulate the state transition:
        conversationStates.put(chatId, ConversationState.TRIP_WAIT_DESTINATION);
        telegramBotService.sendMessage(chatId, "Source location noted: " + sourceAddress + ". Now, what is your destination address?");
    }
    ...
    private void handleTripDestination(long chatId, String telegramId, String destinationAddress) {
        ...
        // 3. Present the user with options (e.g., Route A (90% Safe), Route B (80% Safe)).
        ...
        telegramBotService.sendMessage(chatId, "Destination noted. I've calculated a safe route. Reply 'YES' to start the trip, or 'CANCEL' to abort.");
    }

    private void handleTripConfirmation(long chatId, String telegramId, String confirmation) {
        if (confirmation.equalsIgnoreCase("YES")) {
            // NOTE: This logic REQUIRES TripRepository.
            // If available, the final Trip entity would be created, and an initial notification
            // would be sent to emergency contacts via the NotificationService.

            telegramBotService.sendMessage(chatId, "Trip started! We will monitor your location. We will notify your contacts if you deviate or fail to check in. Use /cancel to end the trip early.");
            conversationStates.put(chatId, ConversationState.IDLE);
