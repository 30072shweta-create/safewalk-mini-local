 ...
 * Responsible for handling low-level Telegram API interactions (sending messages),
 * checking user registration status, and orchestrating the high-level bot commands
...
public class TelegramBotService {
    ...
    // Placeholder for the external Telegram client (since we don't have the library source)
     ...
     * @param restTemplate Used for making HTTP requests to the Telegram API.
     * @param userRepository Repository for accessing user data.
     * @param emergencyContactRepository Repository for accessing emergency contact data.
     */
    ...
    /**
     * Sends a simple text message to a specified Telegram chat using the Telegram API.
    ...
    public boolean sendMessage(long chatId, String text) {
        // NOTE: In a complete project, we would use a proper library like
        // TelegramBots, but for structure purposes, we simulate the HTTP call.
        try {
            String url = String.format("https://api.telegram.org/bot%s/sendMessage", botToken);
            
            // Simplified Payload structure (real client would handle this)
            String payload = String.format("{\"chat_id\": %d, \"text\": \"%s\"}", chatId, text.replace("\"", "\\\""));
            
            // Simulate the call - real code would use restTemplate.postForEntity(url, ...)
            logger.debug("Simulating Telegram API call: POST {} with payload: {}", url, payload);
    ...
    /**
     * Checks if a user is already registered in the system based on their Telegram ID.
    ...
    public void handleStartCommand(long chatId, String telegramId) {
        if (isUserRegistered(telegramId)) {
            ...
            // NOTE: The next line would normally call the ConversationService to start state tracking.
            // Example: conversationService.startRegistration(chatId, telegramId);
            
            sendMessage(chatId, "Welcome to SafeWalk! To get started, please register with us. We need your name and phone number. What is your full name?");
        }
    }
    ...
    public void handleNewTripCommand(long chatId, String telegramId) {
        ...
        sendMessage(chatId, "Starting a new trip setup. Please provide your starting location (e.g., '123 Main St, City').");
    }
