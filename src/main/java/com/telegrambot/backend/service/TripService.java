...
/**
 * Service responsible for all communication with the external SafeWalk Core backend
 * related to trip creation, retrieval, and actions (SOS, End Trip).
 *
 * It uses RestTemplate for synchronous HTTP calls and HmacUtil for request signing.
 */
@Service
public class TripService {
    ...
    public void triggerSos(SosDto sosDto) {
        ...
        try {
            ...
            logger.error("Failed to trigger SOS for Trip {} with SafeWalk Core backend: {}", sosDto.getTripId(), e.getMessage(), e);
        }
    }
    ...
    public void endTrip(Long tripId) {
        ...
        try {
            // Note: DELETE request usually doesn't have a body, so we only sign the URL/path if needed.
            // For simplicity, we assume no body and no signing of the URL path is required here.
    ...
    /**
     * Retrieves route options for a given trip creation request (Source/Destination).
     *
    ...
    public List<RouteOptionDto> getRouteOptions(TripCreationDto tripDto) {
        ...
        try {
           ...
           ResponseEntity<List<RouteOptionDto>> response =
